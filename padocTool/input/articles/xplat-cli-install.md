...
### Installing node.js and npm on Linux distributions that use [dpkg](http://zh.wikipedia.org/wiki/Dpkg) package management
The most common of these distributions use either the [advanced packaging tool (apt)](http://zh.wikipedia.org/wiki/Advanced_Packaging_Tool) or other tools based on the `.deb` package format. Some examples are Ubuntu and Debian.
...
### Installing node.js and npm on Linux distributions that use [rpm](http://zh.wikipedia.org/wiki/RPM_Package_Manager) package management
